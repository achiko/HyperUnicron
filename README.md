# HyperUnicorn Points Lab

Dashboard for exploring generated HyperUnicorn points data.
...
Mock data is generated by [src/MockDataGenerator.ts](/src/MockDataGenerator.ts). The current generator is configured for `30` users and `30` days.

```bash
pnpm exec tsc src/MockDataGenerator.ts --target ES2022 --module NodeNext --moduleResolution NodeNext --outDir .generated --skipLibCheck --noEmit false
node .generated/MockDataGenerator.js
```
...
```text
src/mock-data/
src/mock-data/user-activities/
```

## Project Shape

```text
src/main.tsx                 Dashboard and user detail views
src/styles.css               App styling
src/MockDataGenerator.ts     Mock data generator
src/mock-data/leaderboard.json
src/mock-data/activities.json
src/mock-data/user-activities/
```

## Data Views

The dashboard shows total users, total points, total activities, and a clickable leaderboard. Clicking a user opens a detail view with that user’s records, formulas, and point explanations.
